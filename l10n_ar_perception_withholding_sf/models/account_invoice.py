...
class L10nArAccountInvoice(models.Model):
    ...
    def _compute_amount_perception_sf(self):
        self.amount_perception_sf = sum(line.amount for line in self.tax_line_ids if (line.tax_id.is_perception is True
                                                                                        and line.tax_id.type_aliquot == 'sf'))
    ...
    amount_perception = fields.Monetary(string='Percep. IIBB Santa Fe',store=True, readonly=True,
                        compute='_compute_amount_perception'
                                        )
    amount_perception_sf = fields.Monetary(string='Percep. IIBB Santa Fe', store=True, readonly=True,
                        compute='_compute_amount_perception_sf', track_visibility='always')
    ...
    def get_taxes_perception_values(self):
        taxes_perception = {}
        if not self.partner_id.get_exempt_sf(self.date_invoice, 'perception') and self.company_id.calculate_perc_sf:
            aliquot = self.partner_id._get_sf_update(self.company_id)
            if aliquot:
                taxes = self.env['account.tax'].search([('is_perception', '=', True), ('type_aliquot', '=', 'sf'),
